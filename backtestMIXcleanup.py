...
for n in range(startingIndex, len(priceTables[0].index)):
    ...
    for a in range(0, len(priceTables)):
        ...
        if currentPositions[a] != 0:
            ...
            # a long trade when the price moves up, therefore I'll add a universal stop loss: 10% for long positions, and a tighter 5% for short.
            if not stoppedOut[a]:
            ...
            if lastCrossovers[a] != "unknown":
                ...
                if placeTrades[a]:
                    ...
                    if (
                        ...
                        # and (data["Close"][realDate] > data["LSMA"][realDate])
                    ):
